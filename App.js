...
async function handleShareToChannel(payload, sendToWeb) {
  ...
  try {
    ...
    if (social === Share.Social.INSTAGRAM_STORIES) {
      ...
      if (!installed) {
        sendToWeb?.('SHARE_RESULT', { success: false, platform: key, error_code: 'app_not_installed' });
        return;
      }
    ...
    if (social === Share.Social.INSTAGRAM && isIOS) {
      ...
      try {
        await Share.open({ url: uri, type: mime, filename: `share.${ext}`, failOnCancel: false });
        sendToWeb?.('SHARE_RESULT', { success: true, platform: key, method: 'system_sheet', post_id: null });
      } finally { try { await cleanup(); } catch { } }
      return;
    }
    ...
    const { uri, cleanup } = await ensureLocalFile(file, ext);
    try {
      await Share.open({ url: uri, message: text, title: '공유', type: mime, filename: `share.${ext}`, failOnCancel: false });
      sendToWeb?.('SHARE_RESULT', { success: true, platform: key, post_id: null });
    } finally { try { await cleanup(); } catch { } }
  } catch (err) {
...
async function openManageSubscriptionIOS() {
  ...
  try {
    ...
    return Linking.openURL(ok ? deep : 'https://apps.apple.com/account/subscriptions');
  } catch (e) {
...
const App = () => {
  ...
  function waitIapResult(timeoutMs = 30000) {
  ...
  const beginIap = useCallback((kind, { sku } = {}) => {
    if (Platform.OS !== 'ios') return false;
    if (iapBusyRef.current) {
      logIap('beginIap blocked: busy');
      return false;
    }
    iapBusyRef.current = true;
    iapKindRef.current = (kind === 'subscription' ? 'subs' : kind === 'one_time' ? 'inapp' : kind) || null;
    iapSkuRef.current = sku || null;
    sendIapToWeb('IAP_BUSY', { value: true, kind: iapKindRef.current, sku: iapSkuRef.current, at: Date.now() });
    return true;
  }, [sendIapToWeb]);
  ...
  const endIap = useCallback(() => {
    ...
    sendIapToWeb('IAP_BUSY', { value: false, at: Date.now() });
  }, [sendIapToWeb]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.log('[IAP] prefetch ok', { prodsLen: products?.length, subsLen: subscriptions?.length });
      } catch (e) {
        console.log('[IAP] prefetch ERR', e?.message || e);
      }
    })();
  ...
  useEffect(() => {
    ...
    if (transactionId && finishedTxnIdsRef.current.has(transactionId)) {
      console.log('[IAP] skip finish (already done):', transactionId);
      return;
    }
    ...
    (async () => {
      ...
      try {
        ...
        console.log('[IAP] finishTransaction.CATCH', JSON.stringify({ productId, transactionId, isConsumable, code, msg }));
    ...
    try {
      ...
      switch (data.type) {
        ...
        case 'START_SUBSCRIPTION': {
          ...
          if (!beginIap('subscription', { sku })) {
            sendIapToWeb('SUBSCRIPTION_RESULT', { success: false, platform: 'ios', product_id: sku, error_code: 'busy', message: 'iap_busy' });
            break;
          }
          try {
        ...
        case 'START_ONE_TIME_PURCHASE': {
          ...
          if (!beginIap('one_time', { sku })) {
            sendIapToWeb('PURCHASE_RESULT', { success: false, platform: 'ios', one_time: true, product_id: sku, error_code: 'busy', message: 'iap_busy' });
            break;
          }
          try {
