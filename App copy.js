...
const App = () => {
  ...
  const beginIap = useCallback((tag, extra = {}) => {
    const now = Date.now();
    if (iapBusyRef.current || (now - lastIapTsRef.current) < 800) return false;
    lastIapTsRef.current = now;
    iapBusyRef.current = true;
    iapKindRef.current = (tag === 'one_time') ? 'inapp' : 'subs';
    iapSkuRef.current = extra?.sku || null;
    return true;
  }, []);

  const endIap = useCallback(() => {
    iapBusyRef.current = false;
    iapKindRef.current = null;
    iapSkuRef.current = null;
  }, []);
  ...
  const handleStartSignin = useCallback(async (payload) => {
    ...
    try {
      ...
      if (provider === 'apple') {
        ...
        const redact = (v, keep = 6) => {
          try {
            const s = String(v || '');
            if (s.length <= keep * 2) return s ? s[0] + '***' + s.slice(-1) : '';
        ...
        try {
          ...
          const { user: appleUserId, email, fullName, identityToken, authorizationCode, state } = res || {};
          if (!appleUserId) throw new Error('no_apple_user_id');
          try {
            const credState = await appleAuth.getCredentialStateForUser(appleUserId);
            console.log('[APPLE][CRED_STATE]', credState);
          } catch (e) { console.log('[APPLE][CRED_STATE][WARN]', e?.code, e?.message); }
          safeSend('SIGNIN_RESULT', {
            success: true,
            provider: 'apple',
  ...
  // === latestPurchase 처리 (finishTransaction 포함 + 콘솔 로그) ===
  useEffect(() => {
    if (Platform.OS !== 'ios') return;
  ...
  }, [latestPurchase]); // eslint-disable-line react-hooks/exhaustive-deps


  // === iapError (실패/취소/이미 보유) — 진행중일 때, 종류에 맞춰 1회 전송
  useEffect(() => {
    if (Platform.OS !== 'ios') return;
    if (!iapError) return;
  ...
  const onMessageFromWeb = useCallback(async (e) => {
    try {
      ...
      switch (data.type) {
        ...
        // ✅ IAP 전용 준비 신호: 웹이 결제 결과 수신 준비 완료
        case 'WEB_IAP_READY': {
        ...
        case 'START_SUBSCRIPTION': {
          ...
          if (Platform.OS === 'ios') {
            ...
            sendIapToWeb('SUBSCRIPTION_RESULT', { success: false, platform: 'android', error_code: 'not_supported', message: 'use Android project' });
            endIap();
          }
          break;
        }
        ...
        case 'START_ONE_TIME_PURCHASE': {
          ...
          if (!beginIap('one_time', { sku })) break;
          if (Platform.OS === 'ios') {
            try {
              await fetchProducts({ skus: [sku], type: 'inapp' });
              await requestPurchase({ type: 'inapp', request: { ios: { sku } } });
            } catch (e) {
              sendIapToWeb('PURCHASE_RESULT', {
                success: false, platform: 'ios', one_time: true,
                product_id: sku,
                error_code: e?.code || 'start_failed',
                message: String(e?.message || e),
              });
            ...
            sendIapToWeb('PURCHASE_RESULT', { success: false, platform: 'android', error_code: 'not_supported', message: 'use Android project' });
            endIap();
          }
          break;
        }
        ...
        case 'GET_PUSH_TOKEN': {
          try {
            ...
            sendToWeb('PUSH_TOKEN', {
              token: t, platform: Platform.OS, app_version: APP_VERSION,
              install_id: installId ?? 'unknown',
              ts: Date.now()
            });
          } catch (err) {
            sendToWeb('PUSH_TOKEN', {
              token: '', platform: Platform.OS, app_version: APP_VERSION,
              install_id: installId ?? 'unknown',
              ts: Date.now(), error: String(err?.message || err)
            });
          }
          break;
        }
  ...
  }, [handleCheckPermission, handleRequestPermission, handleStartSignin, handleStartSignout, handleWebError, handleWebReady, sendToWeb, token, installId, connected, fetchProducts, requestPurchase, restorePurchases]);

  const onWebViewLoadStart = useCallback(() => {
    // 새 문서 로드 → IAP 준비 해제(큐잉 모드)
