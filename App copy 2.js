...
const App = () => {
  ...
  async function probeLatestBySnapshot(targetSku) {
    try {
      const arr = await getAvailablePurchases();   // import 되어 있어야 함
 ...
 */
  ...
  //     if (s !== 'active') return;
  //     // 최근 대기 SKU가 있다면(있으면) 스냅샷 재확인
  //     // 간단 버전: 아무거나 최신 스냅샷 발견 시 웹에 성공 통지
  //     const snap = await probeLatestBySnapshot(null);
  ...
  //       sendIapToWeb(snap.isConsumable ? 'PURCHASE_RESULT' : 'SUBSCRIPTION_RESULT', payload);
  //       console.log('[IAP][INAPP][FINAL_SEND]');
  ...
  const beginIap = useCallback((tag, extra = {}) => {
    ...
    //   console.log('[IAP][begin] blocked. busy=', iapBusyRef.current, 'delta=', now - lastIapTsRef.current);
    //   return false;
    // }
    ...
    // console.log('[IAP][begin]', 'kind=', iapKindRef.current, 'sku=', iapSkuRef.current);
    return true;
  }, []);

  const endIap = useCallback(() => {
    // console.log('[IAP][end]', 'kind=', iapKindRef.current, 'sku=', iapSkuRef.current);
    ...
    try {
      ...
      switch (data.type) {
        ...
        case 'START_SUBSCRIPTION': {
          ...
          // Plan-B: 12초 동안 1초 간격으로 스냅샷 폴링
          const racePoll = (async () => {
            for (let i = 0; i < 12; i++) {
              const snap = await probeLatestBySnapshot(sku);
              if (snap && snap.transactionId) {
                const payload = { success: true, platform: 'ios', product_id: snap.productId, transaction_id: snap.transactionId };
                sendIapToWeb('SUBSCRIPTION_RESULT', payload);   // 즉시 웹으로 성공 알림
                notifyIapResult(payload);                       // AND_WAIT 해제
                return payload;
              }
        ...
        case 'START_ONE_TIME_PURCHASE': {
          ...
          // const finalRes = await waitIapResult(120000);
